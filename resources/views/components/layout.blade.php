@props(['role' => 'user', 'title' => 'Dashboard - NeuroSmart'])

@php
    $bodyClass = match($role) {
        'admin' => 'bg-slate-50/50',
        default => 'bg-sky-50/20',
    };
    $headerBorder = match($role) {
        'admin' => 'border-slate-200/80',
...
<body class="min-h-screen flex flex-col {{ $bodyClass }} font-sans antialiased">

    {{-- HEADER --}}
    <header class="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b {{ $headerBorder }} shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="text-right hidden md:block">
                    <span class="text-xs text-slate-400 font-semibold block">Acesso Conveniado</span>
                    <span class="text-sm font-bold text-slate-800">{{ session('nombres') }} {{ session('apellidos') }}</span>
                </div>
                <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-sm">
